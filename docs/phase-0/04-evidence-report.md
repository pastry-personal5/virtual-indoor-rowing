...
|---|---|---|---|
| Unsigned Shipping build and package verification | `d4bad0f2aa97` | Pass | 2026-09-16 14:23–14:24 UTC on the pinned reference host; toolchain fingerprint `ue-5.8.2;xcode-26.1.1;macos-26.6.2;arm64`. All of `make doctor`, native build/tests (13/13), `make format-check`, `make unreal-smoke` (Result: Succeeded), `make unreal-shipping` (BUILD SUCCESSFUL), and `make unreal-package-verify` (`OK unsigned Shipping package`) ran interactively at this one revision. `app_sha256=72896216ffa983544040e398a3ea695b5cbddc320fc3228374abc02eb6c15add`. Note: the Shipping/smoke build wrote an unrelated `AndroidFileServerEditor` default block (including a random per-run `SecurityToken`) into `Config/DefaultEngine.ini`; reverted before recording this row so the revision stayed unmodified. |
...
| Bluetooth probe — fresh Allow | Deferred to Phase 4 (ADR-0009) | Deferred | Not a Milestone 3 or Phase 0 requirement. At Phase 4, record only result state and duration from the schema-v1 result. |
| Bluetooth probe — Deny | Deferred to Phase 4 (ADR-0009) | Deferred | Not a Milestone 3 or Phase 0 requirement. At Phase 4, record only result state and duration from the schema-v1 result. |
| Bluetooth probe — Settings repair then Allow | Deferred to Phase 4 (ADR-0009) | Deferred | Not a Milestone 3 or Phase 0 requirement. At Phase 4, record only result state and duration from the schema-v1 result. |
