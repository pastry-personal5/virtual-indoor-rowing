...
- Real-device sessions journal through `FLocalDataJournalSink`. Under ADR-0012, development and single-user runs use owner-only plaintext SQLite/WAL and do not load a Keychain key. Simulator and automation runs create no database.
...
- **Journal startup.** The owner-only journal opens on the Connect click before discovery is created. ADR-0012 removes the development Keychain dependency, so no key prompt or denial can interrupt a row.
...
- **Durability cadence.** The real-device path sets `FlushIntervalMs = 750` and pre-flushes within a drained adapter batch before accepting an event that crosses that interval. The margin keeps decoded chunks below the one-second QA-003 evidence bound at PM5 rates. Journal write latency is included in the evidence.
...
8. Journal-open behavior after re-running `make unreal-shipping`: confirm the owner-only journal opens without a Keychain prompt and that the journal-unavailable path still handles filesystem failures.
...
- **Legacy sealed journals.** ADR-0012 leaves existing encrypted journals in place and does not rewrite or delete them. Current plaintext development runs do not require their legacy Keychain key; any future migration/readback work must preserve those artifacts explicitly.
...
Owner-run finding (2026-09-20): a six-minute real-PM5 row exposed two implementation defects: a monitor-side Just Row end was classified as an aborted app session, and the reviewed profile did not subscribe to Additional Status 2 (`0x0033`), leaving HUD power unavailable. The follow-up changes classify a `Terminated` state after an active Just Row session as a completed local session and add the documented 20-byte `0x0033` average-power stream to profile v7. The run remains incomplete Phase 1 evidence and must be repeated on the rebuilt package.

Owner-run finding (2026-09-20): stroke rate, pace, power, and heart rate occasionally flickered to placeholders during an otherwise healthy real-PM5 row. The PM5 adapter correctly publishes a sparse General Status sample when its optional companion notification arrives outside the bounded merge window; the HUD had rendered that momentary absence as a new `--` value. The display now retains only the last actual PM5 value for up to 1.5 seconds when an optional field is temporarily absent in the same live session. It never retains beyond that bound, across a new session, after an explicit PM5 zero/invalid pace, or over a stale/reconnecting link; official snapshots and journal samples remain sparse and unchanged. `workout_display_retains_live_device_values_across_sparse_samples` covers the policy. A rebuilt packaged real-PM5 visual check remains required.
...
3. Expected on first Connect: one Bluetooth prompt and no Keychain prompt; the owner-only plaintext journal opens before discovery.
